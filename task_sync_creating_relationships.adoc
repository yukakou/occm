...
.Steps for Cloud Volumes ONTAP or Azure NetApp Files

Use these steps to sync data to or from Cloud Volumes ONTAP or to or from Azure NetApp Files.

. In Cloud Manager, click *Canvas*.

. Select a Cloud Volumes ONTAP or Azure NetApp Files working environment.
...
.Steps for all other types of relationships

Use these steps to sync data to or from any other type of supported storage. The steps below provide an example that shows how to set up a sync relationship from an NFS server to an S3 bucket.
