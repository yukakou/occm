---
sidebar: sidebar
permalink: task_getting_started_azure_gov.html
...
summary: Getting started with Cloud Volumes ONTAP includes preparing your Azure environment, launching the OnCommand Cloud Manager software from NetApp Cloud Central, and then launching Cloud Volumes ONTAP using Cloud Manager.
---

= Getting started with Cloud Volumes ONTAP in Azure US Gov regions
:toc: macro
:toclevels: 1
:hardbreaks:
:nofooter:
...
[.lead]
Getting started with Cloud Volumes ONTAP (formerly ONTAP Cloud) includes preparing your Azure environment, launching the OnCommand Cloud Manager software from NetApp Cloud Central, and then launching Cloud Volumes ONTAP using Cloud Manager.

IMPORTANT: Getting started in the Azure US Gov regions is different than other regions because Managed Service Identities are not supported in the US Gov regions. This section describes how to grant the required permissions by creating and setting up a service principal in Azure Active Directory and by obtaining the credentials that Cloud Manager needs.
...
Cloud Manager is now installed and set up so users can deploy Cloud Volumes ONTAP in Azure.
