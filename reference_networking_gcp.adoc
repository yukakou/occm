...
Four VPCs are required for an HA pair and its four network interfaces.
+
Four VPCs are required because GCP requires that each network interface resides in a separate VPC network.
+
...
+
Similar to a single node system, an HA pair is supported in a shared VPC and also in non-shared VPCs. However, only VPC-0 can be a shared VPC. All other VPCs must be a non-shared VPC.
...
+
* One set for HA components in VPC-0 that includes rules that enable data access to Cloud Volumes ONTAP. <<Firewall rules for Cloud Volumes ONTAP,Learn more>>.
* Another set for HA components in VPC-1, VPC-2, and VPC-3 that are open for inbound & outbound communication between the HA components. <<Firewall rules for VPC-1, VPC-2, and VPC-3,Learn more>>.
...
Cloud Manager creates GCP firewall rules that include the inbound and outbound rules that Cloud Volumes ONTAP need to operate successfully. You might want to refer to the ports for testing purposes or if you prefer your to use own security groups.
