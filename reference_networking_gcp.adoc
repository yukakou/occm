...
Four VPCs and four subnets are required for an HA pair, the mediator, and the HA pair's four network interfaces. Four VPCs are required because GCP requires that each network interface resides in a separate VPC network.
+
