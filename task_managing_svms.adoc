...
=== Create storage VMs on a single node system
...
=== Create storage VMs on an HA pair in a single AZ
...
=== Create storage VMs on an HA pair in multiple AZs

These steps create a new SVM with one NFS LIF and one storage VM management LIF. This configuration requires one unused secondary private IP on eth0 (e0a) for each node.
...
== Manage storage VM disaster recovery
