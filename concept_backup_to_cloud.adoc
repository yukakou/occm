...
** The on-prem cluster must be running ONTAP 9.7 or later.
** Cloud Manager must be deployed on Azure. There is no support for on-premises Cloud Manager deployments.
** The destination location for backups is only object storage on Azure.
** Backups can be restored only to Cloud Volumes ONTAP systems deployed on Azure. You cannot restore a backup to an on-premises ONTAP system or to a Cloud Volumes ONTAP system that is using a different cloud provider.
