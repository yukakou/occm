...
Backup to Cloud enables you to create volume backups from the following working environments:
...
| AWS | Cloud Volumes ONTAP in AWS | Cloud Volumes ONTAP in AWS
 On-premises ONTAP system
| Azure | Cloud Volumes ONTAP in Azure | Cloud Volumes ONTAP in Azure
On-premises ONTAP system
| Google | Cloud Volumes ONTAP in Google | Cloud Volumes ONTAP in Google
...
The Restore service (for the single file restore functionality) does not require a license of any type. The only requirement is that you have a valid subscription to use the Backup to Cloud service. The only cost for the Restore service is the cost to your cloud provider for the deployed instance.
...
* In Azure, the Restore virtual machine runs on ...
...
** Backups can be restored only to Cloud Volumes ONTAP systems deployed on the same cloud provider or to an on-premises ONTAP system.
