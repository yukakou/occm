...
You can restore a volume or a files from a backup file to the following working environments:

[cols=2*,options="header",cols="50,50"]
|===

| Backup from Source System
...
| Cloud Volumes ONTAP in Google Cloud *
| Google
| On-premises ONTAP backup files in AWS
| AWS
On-prem system
| On-premises ONTAP backup files in Azure
| Azure
On-prem system

|===

* Only volume restore is supported from a Google backup file at this time - there is no current support for single file restore.
