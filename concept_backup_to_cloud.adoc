...
For BYOL you'll pay NetApp to use the service for a period of time, say 6 months, and for a maximum amount backup capacity, say 1 GB. You'll receive a serial number, per node, that you enter in the Cloud Manager Licensing page. When either limit is reached you'll need to renew the license. Go link:task_modifying_ontap_cloud.html#installing-license-files-on-cloud-volumes-ontap-byol-systems[here to see how to apply the license] on a Cloud Volumes ONTAP system.
