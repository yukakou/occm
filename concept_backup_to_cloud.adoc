...
** Cloud Manager must be deployed on Azure or AWS. There is no support for on-premises Cloud Manager deployments.
** The destination location for backups is only object storage on Azure or AWS.
