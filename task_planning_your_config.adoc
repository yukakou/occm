...
EBS disk size::
You need to choose an initial disk size when you launch a Cloud Volumes ONTAP system. After that, you can let Cloud Manager manage a system's capacity for you, but if you want more control, be aware of the following:
...
Look at the supported virtual machine types in the http://docs.netapp.com/cloud-volumes-ontap/us-en/index.html[Cloud Volumes ONTAP Release Notes^] and then review details about each supported VM type:
...
+
HA systems use Premium Storage page blobs--you don't need to choose between different disk types. Meanwhile, single node systems can use two types of Azure Managed Disks:
