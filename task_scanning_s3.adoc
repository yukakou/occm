...
When Cloud Manager is running in the AWS account that has the S3 buckets you want to scan, it automatically discovers the S3 buckets and displays them in an Amazon S3 working environment. Later, you can configure Cloud Compliance to <<Scanning buckets from additional AWS accounts,scan S3 buckets that are in different AWS accounts>>.
...
TIP: You can also enable compliance scans for a working environment from the Scan Configuration page by clicking the image:screenshot_gallery_options.gif[] icon and selecting *Deactivate Compliance*.
...
.. Create a policy that includes the permissions listed below.
+
[source,json]
{
    "Version": "2012-10-17",
