...
* A single-node ONTAP Cloud system can have to 35 data disks.
...
Several EC2 instance types will not be supported with the upcoming release of ONTAP Cloud 9.4 when creating new systems. Existing ONTAP Cloud systems running these instance types will continue to operate normally; however, NetApp strongly recommends changing to a different instance type.
