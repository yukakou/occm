...
== Cloud Manager 3.8.7 (2 Aug 2020)
...
We have fully introduced a software-as-a-service experience for Cloud Manager. This new experience makes it easier for you to use Cloud Manager and enables us to provide additional features to manage your hybrid cloud infrastructure. link:concept_saas.html[Learn more].

Cloud Manager includes a https://cloudmanager.netapp.com/[SaaS-based interface] that is integrated with NetApp Cloud Central, and Connectors that enable Cloud Manager to manage resources and processes within your public cloud environment. (The Connector is actually the same as the existing Cloud Manager software that you're used to—​it includes the same user interface and the same management layer.)
...
* *Encrypt Azure managed disks and storage blobs using external keys*
+
You can now encrypt Azure managed disks and storage blobs on single node Cloud Volumes ONTAP systems using external keys from another account. This feature is supported using APIs.
+
...
+
The Cloud Compliance instance is set up and deployed using a new wizard in Cloud Manager instead of deploying it separately within one of your working environments. After deployment is complete you enable the service for each working environment you want to scan.
...
Note that the initial configuration that can be deployed using Cloud Manager must meet the following requirements. Other configurations continue to be deployed using the legacy procedures.
...
+
Other storage platforms and other cloud providers are not supported at this time.
