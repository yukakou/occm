...
== Cloud Manager 3.8.6 (5 July 2020)
...
You can now purchase multiple licenses for a Cloud Volumes ONTAP BYOL system to allocate more than 368 TB of capacity. For example, you might purchase two licenses to allocate up to 736 TB to Cloud Volumes ONTAP. Or you could purchase four licenses to get up to 1.4 PB.

The number of licenses that you can purchase for a single node system or HA pair is unlimited.

Be aware that disk limits prevent you from reaching the capacity limit by using disks alone. You can reach the capacity limit by link:concept_data_tiering.html[tiering inactive data to object storage^]. For information about disk limits, refer to https://docs.netapp.com/us-en/cloud-volumes-ontap/[storage limits in the Cloud Volumes ONTAP Release Notes^].

=== Support for iSCSI volumes

Cloud Manager now enables you to create iSCSI volumes for Cloud Volumes ONTAP and on-prem ONTAP clusters directly from its user interface.

When you create an iSCSI volume, Cloud Manager automatically creates a LUN for you. We’ve made it simple by creating just one LUN per volume, so there’s no management involved. After you create the volume, link:task_provisioning_storage.html[use the IQN to connect to the LUN from your hosts].

NOTE: You can create additional LUNs in the volume from System Manager or the CLI.

=== Support for the All tiering policy with Cloud Volumes ONTAP
...
* Ability to choose the volumes to be scanned within a working environment
+
Now you can disable scanning for individual volumes in a Cloud Volumes ONTAP or Azure NetApp Files working environment if those volumes don't need to be scanned for compliance. link:task_getting_started_compliance.html[Learn more about disabling scanning for volumes].

* HIPAA Report
+
A new Health Insurance Portability and Accountability Act (HIPAA) Report is now available. This report is designed to aid in your organization’s requirement to comply with HIPAA data privacy laws. link:task_generating_compliance_reports.html#hipaa-report[Learn more about the HIPAA report].
...
+
Learn more about the new Backup to Cloud BYOL license.

* Combined Policy and Retention column
+
The Policy & Retention column provides an easier way to see the number and type of backups that will be created for each volume in your working environments.
