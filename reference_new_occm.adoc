...
If you have an AWS Outpost, you can deploy Cloud Volumes ONTAP in that Outpost by selecting the Outpost VPC. The experience is the same as any other VPC that resides in AWS. Note that you will need to first deploy a Connector in your AWS Outpost.
...
* A new Cloud Performance Test gives you the ability to measure network latency and throughput performance from an ONTAP cluster to an  object store before and after you set up data tiering.
+
...
+
In the Help menu, a new Support Dashboard includes links to resources that can enable you to get help, submit feedback, and contact NetApp Support. Access tools in the *Connector AutoSupport* tab to send and download AutoSupport messages.
+
...
+
For example, the following image shows an example of two working environments where data is backed up from Cloud Volumes ONTAP to Amazon S3, and where data is synced between Amazon S3 and Cloud Volumes ONTAP.
+
