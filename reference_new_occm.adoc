...
====
* https://docs.netapp.com/us-en/occm34/reference_new_occm.html[What's new in Cloud Manager 3.4]
* https://library.netapp.com/ecm/ecm_download_file/ECMLP2735470[What's new in Cloud Manager 3.3]
====
...
IMPORTANT: Changes in this release require a new Azure permission for Cloud Manager. Read the changes below.
...
You can now enable WORM storage on new Cloud Volumes ONTAP systems. Write once, read many (WORM) storage enables you to retain critical files in unmodified form for regulatory and governance purposes and to protect from malware attacks. WORM files are protected at the file level.

link:pending.html[Learn more about WORM storage].

=== Support for the auto tiering policy with Cloud Volumes ONTAP Standard

The _auto_ volume tiering policy is now supported with Cloud Volumes ONTAP Standard. You can select the auto tiering policy when creating, modifying, or replicating a volume. Data tiering enables automated tiering of cold data to low-cost object storage.

link:concept_storage.html#data-tiering-overview[Learn more about data tiering].

=== Cloud Manager deployment in Azure from NetApp Cloud Central

Starting soon, you can deploy new Cloud Manager systems in Microsoft Azure directly from https://cloud.netapp.com[NetApp Cloud Central]. Cloud Central simplifies the deployment experience and provides a single location to view and manage multiple Cloud Manager systems. Providing credentials to Cloud Central is secure and private—it does not save the credentials. It uses them only to deploy the virtual machine.
