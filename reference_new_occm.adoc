...
In previous releases, modifying the instance type for a Cloud Volumes ONTAP HA system could fail if locks were present on SMB/CIFS files.
Starting in this release, Cloud Manager can detect and break the locks before it proceeds with the instance change. You simply need to confirm when prompted.
