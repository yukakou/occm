...
You can now deploy new ONTAP Cloud 9.4 systems from Cloud Manager and upgrade your existing systems to the 9.4 release. Cloud Manager also provides support for https://clouddocs.netapp.com/ontapcloud/en/reference_new_94.html[the new features introduced in ONTAP Cloud 9.4].
////
=== New permissions required for ONTAP Cloud 9.4

Cloud Manager will require new permissions for key features in the upcoming ONTAP Cloud 9.4 release. To ensure that your Cloud Manager systems are ready to deploy and manage ONTAP Cloud 9.4 systems, NetApp recommends that you update your Cloud Manager policy now by adding the following permissions:
