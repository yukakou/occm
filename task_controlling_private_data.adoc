...
Additionally, if you have added a database server to be scanned, the _Data Fusion_ feature allows you to scan your files to identify whether unique identifiers from your databases are found in those files. See <<Creating custom personal data identifiers from your databases,Creating custom personal data identifiers from your databases>> for details.
...
A feature we call _Data Fusion_ allows you to scan your files to identify whether unique identifiers from your databases are found in those files - basically making your own list of "personal data" that is identified in Cloud Compliance scans. You can choose the identifiers by selecting a specific column, or columns, in a database table. This gives you the full picture about where potential sensitive data resides in _all_ your files.

For example, the diagram below shows how data fusion is used to scan your volumes and buckets for occurrences of all your Customer IDs from your Oracle database.
...
After the next scan these results appear in the Dashboard under the "Personal" results section, and in the Investigation page in the "Personal Data" filter. Each source column you added appears in the filter list as "Database.Schema.Table.Column", for example `Oracle1.Accounts.Customers.Customer ID`.

Note that if you later decide not to scan your files using a certain Data Fusion source, you can select the source row from the Data Fusion inventory page and click *Delete Data Fusion Source*.
...
* The left-pane filters enable you to refine the results by working environment, storage repository, category, private data, file type, last modified date, whether the S3 object’s permissions are open to public access, etc...
