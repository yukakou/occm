...
You can ensure the high availability of your data by deploying an ONTAP Cloud HA configuration across multiple Availability Zones (AZs) or in a single AZ. You should review more details about each configuration to choose which best fits your needs.
...
For details, see link:reference_networking_aws.html#networking-requirements-for-ontap-cloud-ha-in-multiple-azs[Networking requirements for ONTAP Cloud HA in multiple AZs].
...
For more details about ONTAP Cloud performance, see link:concept_performance.html[Performance].
