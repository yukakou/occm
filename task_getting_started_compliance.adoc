...
Note that Cloud Compliance must be deployed in the cloud when scanning Azure NetApp Files volumes. Cloud Compliance can be deployed in the cloud or in an on-premises location when scanning Cloud Volumes ONTAP or on-premises ONTAP systems.
...
You can scan data on your on-premises ONTAP systems directly using Cloud Compliance. You can also replicate the on-prem NFS or CIFS data by using SnapMirror to a Cloud Volumes ONTAP working environment and then enable compliance for those data protection volumes. Choose whichever way works best for you.

TIP:: We recommend that you link:task_discovering_ontap.html[discover your on-prem ONTAP systems^] in Cloud Manager to add them to a working environment so that you can  scan data on those system directly from Cloud Compliance.
