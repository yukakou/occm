...
To scan volumes for Azure NetApp Files, link:task_manage_anf.html[Cloud Manager must be set up to discover the configuration].
...
. On the *Scan Status* page, review the scan status for each CIFS and NFS volume to identify any errors that you can correct.
