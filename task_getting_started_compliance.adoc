...
* Ensure that users can access the Cloud Manager interface from a host that has a direction connection to AWS or Azure, or from a host that's inside the same network as the Cloud Compliance instance (the instance will have a private IP address).
...
* Volume export policies must allow access from the Cloud Compliance instance.
...
* If Cloud Manager doesn’t use a proxy server to access the internet in AWS, the security group for Cloud Manager must allow inbound traffic over TCP port 3128 from the Cloud Compliance instance.
...
. Ensure that volume export policies include the IP address of the Cloud Compliance instance so it can access the data on each volume.
...
. In AWS, open an additional port if Cloud Manager doesn't use a proxy server to access the internet.
+
In this type of configuration, the security group for Cloud Manager must allow inbound traffic over TCP port 3128 from the Cloud Compliance instance.
+
