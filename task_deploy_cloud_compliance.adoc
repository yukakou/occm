...
| Provides access to software images and templates.
|===
...
. Download the Cloud Compliance software from the https://mysupport.netapp.com/site/products/all/details/cloud-compliance/downloads-tab[NetApp Support Site^], and then copy it to the Linux host (using `scp` or some other method).
...
.. Enter proxy details as prompted by the installer. If your Cloud Manager already uses a proxy, there is no need to enter this information again here since Cloud Compliance will automatically use the proxy used by Cloud Manager.
+
...
The Cloud Compliance installer installs packages, installs docker, registers the installation, and installs Cloud Compliance. Installation can take up to 10 to 20 minutes. If there is connectivity over port 8080 between the host machine and the Connector instance, you will see the installation progress in the Compliance tab in Cloud Manager.
