...
[role="quick-margin-para"]
Launch the installation wizard to deploy the Cloud Compliance instance. You can deploy Cloud Compliance in the cloud or in an on-premises location.
...
* Databases can be scanned when using either Connector.
* On-prem ONTAP systems can be scanned when using either Connector.
...
.. Paste the information you copied from step 3 after you clicked “Deploy Compliance On Premises”.
+
...
.. Proxy details if needed. If your Cloud Manager already uses a proxy, there is no need to enter this information again here since Cloud Compliance will automatically use the proxy used by Cloud Manager:
+
...
+
_cc_host_ = IP or host name of the host machine, as it will be accessed by the Connector instance
+
...
The installer will start installing Cloud Compliance (installs packages, installs docker, registers the installation, downloads the Cloud Compliance artifact, and installs Cloud Compliance).

* If there is connectivity over port 8080 between the host machine and the Connector instance, you should see the installation progress in the “Compliance” tab in Cloud Manager.
* Installation takes up to 10 minutes. When it’s done, the “Compliance” tab should show the Cloud Compliance UI’s Configuration page, where you can now select which data sources to scan.
