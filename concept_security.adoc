...
You can enable NetApp Volume Encryption on a new or existing volume by using the CLI. Cloud Manager does not support NetApp Volume Encryption. For instructions, see link:task_encrypting_volumes.html[Encrypting volumes with NetApp Volume Encryption].
