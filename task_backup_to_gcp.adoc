...
[role="quick-margin-para"]
From the Backup List, select a volume, select a backup, and then restore data from the backup to a new volume. A Service Account is required on the Cloud Volumes ONTAP system where you are preforming the restore.
