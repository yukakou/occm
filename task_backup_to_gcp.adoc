...
. *Details & Credentials*: Select the project if different than the default Project (where Cloud Manager resides), specify a cluster name, enable the Service Account switch and select the Service Account (to enable backups and tiering), and then specify credentials.
+
