...
HA configurations in multiple AZs use floating IP addresses for NAS client access from within the VPC. These IP addresses can migrate between nodes when failures occur. They are not natively accessible to clients that are outside of the VPC, unless you link:task_setting_up_transit_gateway.html[set up an AWS transit gateway] after you deploy the HA system.
+
...
After you deploy an HA system in multiple AZs, link:task_setting_up_transit_gateway.html[set up an AWS transit gateway] to enable access to an HA pair's floating IP addresses from outside the VPC where the HA pair resides.
