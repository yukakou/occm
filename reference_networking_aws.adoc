...
HA configurations in multiple AZs use floating IP addresses for NAS client access. These IP addresses can migrate between nodes when failures occur. They are not natively accessible to clients that are outside of the VPC, unless you link:task_setting_up_transit_gateway.html[set up an AWS transit gateway].
+
