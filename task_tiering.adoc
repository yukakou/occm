...
+
Cloud Manager creates an object store for cold data when the system is created, as long as there are no connectivity or permissions issues. After that, you just need to enable data tiering on volumes (and in some cases, <<Ensuring that tiering is enabled on aggregates,on aggregates>>).
====
...
=== Enabling data tiering after fixing an issue

Cloud Manager creates an object store for cold data when the system is created, as long as there are no connectivity or permissions issues. You can implement the requirements described above and then enable tiering, which creates the object store.

.Steps

. <<Cloud provider requirements,Ensure that you've met all requirements>>.
