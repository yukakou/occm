...
link:concept_licensing.html[Learn more about licenses].
...
Go to the https://cloud.netapp.com/cloud-sync-service[Cloud Sync page on NetApp Cloud Central^] and click *Start Free Trial*. Log in or sign up to Cloud Central. After you’ve authenticated, you’re ready to get started using the Cloud Sync service.
...
* link:reference_networking_overview.html[Review the networking overview]
* link:reference_endpoints.html[Review the list of endpoints that the data broker contacts]
...
link:task_managing_relationships.html#changing-the-settings-for-a-sync-relationship[Learn how to change the settings for a sync relationship].
...
* Cloud Sync supports data-in-flight encryption between source and target NFS servers. link:task_nfs_encryption.html[Learn more].
...
You can set up Cloud Sync to preserve access control lists (ACLs) between a source SMB share and a target SMB share. Or you can manually copy the ACLs yourself. link:task_copying_acls.html[Learn how to copy ACLs between SMB shares].
...
Then you can consider accelerating sync performance by adding an additional data broker to share the load of that relationship. link:task_managing_relationships.html#accelerating-sync-performance[Learn how to accelerate sync performance].
