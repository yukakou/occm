...
.. If you want to deploy ONTAP Cloud systems in AWS, set up an IAM role that includes the required permissions.
+
link:task_granting_aws_permissions.html[Granting permissions when Cloud Manager is not launched from Cloud Central]

.. If you want to deploy ONTAP Cloud systems in Azure, create and set up a service principal in Azure Active Directory.
+
