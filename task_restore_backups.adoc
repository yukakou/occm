...
. In the _Select Source_ page, select the *Working Environment*, the *Volume*, and the *Snapshot* that has the date/time stamp that you want to restore.
+
...
. Then select the Storage VM where the volume will reside and enter the name for the new volume that you want to restore.
+
...
+
You can specify the Aggregate that the volume will use when restoring a volume to an on-prem ONTAP system.
...
NOTE: Restoring files from Google Cloud Storage is currently not supported.
...
. When you want to restore one or more files from a volume, select the Restore tab and select the backup file in which the file resides.
+
...
. You choose the file (or up to 8 files) from that backup, select the location where you want the file(s) to be restored (the working environment, volume, and folder), and click Restore.
+
