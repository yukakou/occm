...
In GCP, Cloud Volumes ONTAP HA configurations include the following components:
...
* Four Virtual Private Clouds (VPCs) for the HA pair's four network interfaces.
+
...
* The recovery time objective (RTO) is 60 seconds for NFS/CIFS and 120 seconds for iSCSI.
+
In the event of an outage, data should be available by the RTO.

=== HA deployment models

You can ensure the high availability of your data by deploying an HA configuration in multiple zones or a single zone.
