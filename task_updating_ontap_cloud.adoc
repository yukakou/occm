...
NOTE: Upgrades of HA pairs are nondisruptive--HA pairs continue to serve data during the upgrade process. Cloud Manager might upgrade the HA mediator as part of this process.
Meanwhile, the upgrade of a single-node system takes the system offline for up to 25 minutes, during which I/O is interrupted.
