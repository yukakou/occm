...
Supported regions::
On-prem backup is supported in all Azure and AWS regions https://cloud.netapp.com/cloud-volumes-global-regions[where Cloud Volumes ONTAP is supported^].
...
When using Amazon S3, the IAM role that provides Cloud Manager with permissions must include S3 permissions from the latest https://mysupport.netapp.com/site/info/cloud-manager-policies[Cloud Manager policy^], plus two additional permissions.

.Steps

. Provide the standard permissions to the IAM user:
+
...
. Add two additional permissions:
+
[source,json]
"s3:PutObject",
"s3:DeleteObject"
...
- For Azure, enter:
.. The Azure subscription used to store the backups and the Azure region.
...
- For AWS, enter:
.. The AWS credentials (Access Key and Secret Key) used to store the backups.
.. The IPspace in the ONTAP cluster where the volumes you want to back up reside.
+
