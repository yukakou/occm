...
. *Services*: Select the services that you want to use on this system. In order to select Backup to Cloud, or to use Tiering, you must have specified the Service Account in step 3.
...
| Service Account Name | If you plan to use the link:concept_data_tiering.html[Tiering] or link:concept_backup_to_cloud.html[Backup to Cloud] services, you need to enable the *Service Account* switch and then select the Service Account that has the predefined Storage Admin role.
